...
# --- 2. API & データベース初期化 ---
try:
    # Gemini API (最新の 2.5 Flash モデルを使用)
...
# データの取得（インデックスの安全性を確保）
current_q_idx_safe = current_q_idx % len(df)
q_data = df.iloc[current_q_idx_safe]
question = q_data["question"]
correct_answer = q_data["answer"]

st.markdown(f"<div class='status-box'>ルーム: {st.session_state.room_id} | 参加者: {st.session_state.user_id}</div>", unsafe_allow_html=True)
...
if st.button("✅ 回答を送信", type="primary", use_container_width=True):
    if canvas_result.image_data is not None:
        with st.spinner("AIが採点中..."):
            try:
                # 透明度を含むRGBAから白背景のRGBに変換（AIの視認性向上）
                ...
                prompt = (
                    ...
                    "判定は厳格に行ってください。正解の場合は必ず『正解』という言葉を含めて回答し、"
                    "不正解の場合は理由を教えてください。"
                )
...
# 勝者表示
try:
    ans_res = supabase.table("answers").select("user_id").eq("room_id", st.session_state.room_id).eq("question_idx", current_q_idx).order("solved_at", descending=False).execute()
    if ans_res.data:
